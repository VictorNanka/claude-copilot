        ...
        start: () => {
            ...
            if (startedServer) {
                vscode.window.showInformationMessage(
                    "LM API server is already running"
                );
                return;
            }
            ...
            vscode.window.showInformationMessage(
                `LM API server is running on port ${port}`
            );
        },
        stop: () => {
            if (startedServer) {
...
async function registerMCPToolsToVSCode(mcpManager: MCPManager, clientName?: string): Promise<void> {
...
function applySystemPromptFormat(messages: Array<OpenAIMessage | AnthropicMessage>, systemContent: string, format: SystemPromptFormat): Array<OpenAIMessage | AnthropicMessage> {
    ...
    switch (format) {
        case 'merge':
            ...
            const firstUserContent = typeof firstMessage.content === 'string' 
...
function extractToolNamesFromRequest(body: any): string[] {
...
async function ensureToolsRegistered(toolNames: string[], mcpManager: MCPManager): Promise<void> {
    ...
    for (const toolName of toolNames) {
        ...
        const mcpTools = mcpManager.getAvailableTools();
        const mcpTool = mcpTools.find(tool => tool.name === toolName || `${toolName}`.includes(tool.name));
...
interface RetryContext {
    ...
    onToolResult?: (callId: string, toolName: string, result: vscode.LanguageModelToolResult) => Promise<void> | void;
    retryCount: number;
}
...
async function executeStreamWithRetry(contextId: string, maxRetries: number): Promise<void> {
    ...
    try {
        const chatResponse = await context.model.sendRequest(context.chatRequest, context.requestOptions);

        // Create enhanced tool result handler that can trigger retries
        const retryableOnToolResult = context.onToolResult ? async (callId: string, toolName: string, result: vscode.LanguageModelToolResult) => {
            const resultText = result.content.map(part =>
            ...
            if (resultText.includes('dynamically discovered and registered') && context.retryCount < maxRetries) {
                winstonLogger.info(`🔄 Retrying request after tool discovery (attempt ${context.retryCount + 1}/${maxRetries})`);
                context.retryCount++;
...
function tryCommonToolPatterns(toolName: string): ToolSignature | null {
    const commonPatterns: { [key: string]: ToolSignature } = {
        ...
        'cp': { name: 'cp', description: 'Copy files', parameters: { type: 'object', properties: { source: { type: 'string' }, dest: { type: 'string' } }, required: ['source', 'dest'] } },
        'mv': { name: 'mv', description: 'Move files', parameters: { type: 'object', properties: { source: { type: 'string' }, dest: { type: 'string' } }, required: ['source', 'dest'] } },
        'rm': { name: 'rm', description: 'Remove files', parameters: { type: 'object', properties: { path: { type: 'string' } }, required: ['path'] } },

        // Development tools
        'git': { name: 'git', description: 'Git version control', parameters: { type: 'object', properties: { command: { type: 'string' } }, required: ['command'] } },
        'npm': { name: 'npm', description: 'Node package manager', parameters: { type: 'object', properties: { command: { type: 'string' } }, required: ['command'] } },
        'python': { name: 'python', description: 'Python interpreter', parameters: { type: 'object', properties: { code: { type: 'string' } }, required: ['code'] } },
...
async function processStreamChunks(
    ...
    for await (const chunk of chatResponse.stream) {
        if (chunk instanceof vscode.LanguageModelTextPart) {
            ...
            winstonLogger.info(
                `🔧 TOOL CALL: ${chunk.name}, CallId: ${chunk.callId}`
            );
            ...
            if (onToolResult) {
                try {
                    ...
                    const errorMessage = error instanceof Error ? error.message : String(error);
                    if (errorMessage.includes('tool') && (errorMessage.includes('not found') || errorMessage.includes('not registered'))) {
                        winstonLogger.info(`🔍 Attempting to discover and register missing tool: ${chunk.name}`);
                        ...
                        if (discoveredTool) {
                            ...
                            if (context) {
                                ...
                                if (added) {
                                    ...
                                    if (registered) {
                                        winstonLogger.info(`🔧 Successfully discovered and registered tool: ${chunk.name}`);
...
function newHonoServer(getConfig: () => Config, mcpManager: MCPManager) {
    ...
    app.post("/chat/completions", async (c) => {
        try {
            ...
            const model = await findModelWithFallback(
                body.model,
                config.defaultModel
            );
            if (!model) {
    ...
    app.post("/v1/messages", async (c) => {
        try {
            ...
            if (body.stream) {
                ...
                return stream(c, async (stream) => {
                    await stream.write(
                        `event: message_start\ndata: {"type":"message_start","message":{"id":"msg_${Date.now()}","type":"message","role":"assistant","model":"${model.id
            ...
            return c.json(
                createErrorResponse("Internal server error", 500, "api_error"),
                500
            );
        }
    });

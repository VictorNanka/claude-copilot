...
export async function activate(context: ExtensionContext): Promise<void> {
	...
	const startLmApiServerDisposable = vscode.commands.registerCommand('http-lm-api.startLmApiServer', async () => {
		await server.start();
	});

	const stopLmApiServerDisposable = vscode.commands.registerCommand('http-lm-api.stopLmApiServer', async () => {
		await server.stop();
	});
...
function registerClaudeCodeTools(context: ExtensionContext): void {
	...
	const expectedTools = ["Task", "Bash", "Glob", "Grep", "LS", "exit_plan_mode", 
